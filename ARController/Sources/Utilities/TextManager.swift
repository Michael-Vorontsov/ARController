...
public protocol TextManaging: class {
    var textManager: TextManager? { get set }
}

public protocol TextManagerContaining: DebugVisualizing {
    
    func present(_ viewControllerToPresent: UIViewController, animated flag: Bool, completion: (() -> Swift.Void)?)
    var view: UIView! { get }
    var messageLabel: UILabel! { get set }
    var messagePanel: UIView! { get set }
    var debugMessageLabel: UILabel! { get set }
}

public class TextManager {
	
	public init(viewController: TextManagerContaining) {
	...
	public func showDebugMessage(_ message: String) {
	...
	public var schedulingMessagesBlocked = false
	...
	public func escalateFeedback(for trackingState: ARCamera.TrackingState, inSeconds seconds: TimeInterval) {
		...
		self.trackingStateFeedbackEscalationTimer = Timer.scheduledTimer(withTimeInterval: seconds, repeats: false, block: { _ in
			...
			let restartAction = UIAlertAction(title: "Reset", style: .destructive, handler: { _ in
				(self.viewController as? ErrorReporting)?.restartExperience(self)
	...
	public let blurEffectViewTag = 100
	...
	// MARK: - Private
	private var viewController: TextManagerContaining!
